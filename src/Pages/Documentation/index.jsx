...
function Documentation() {
  ...
  async function callsHandler() {
    const response = await fetch("https://jsonplaceholder.typicode.com/todos");
    const calls = await response.json();
    setTotalCall(calls.length);
  }
  async function incomingHandler() {
    const response = await fetch("https://jsonplaceholder.typicode.com/posts");
    const incoming = await response.json();
    setIncomingCall(incoming.length);
  }
  async function outgoingHandler() {
    const response = await fetch("https://jsonplaceholder.typicode.com/albums");
    const outgoing = await response.json();
    setOutgoingCall(outgoing.length);
  }
  const total = [totalUser, totalCall, incomingCall, outgoingCall];
  console.log(total);
  return (
    <>
      {total.map((el) => {
